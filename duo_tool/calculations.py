...
def monthly_interest_rate(interest: float):
    """Calculates the monthly interest rate, including compounding interest."""
    return (1 + interest) ** (1 / 12) - 1
...
def monthly_payment(remaining_debt: float, interest: float, months: int) -> float:
    ...
    monthly_interest_r = monthly_interest_rate(interest)
    payment = (
        remaining_debt
        * monthly_interest_r
        * (1 + monthly_interest_r) ** months
        / ((1 + monthly_interest_r) ** months - 1)
    )
    return round(payment, 2)


def aanloopfase(
    start_date: Timestamp, interest_rate: float, original_debt: int, payment_offset: int
...
def one_time_payment(
    ...
    if payment_date > start_date + pd.DateOffset(months=payment_offset - 1):
        ...
        updated_payment, rest_df = payment_phase(remaining_debt, interest_rate, months_left, payment_date)
