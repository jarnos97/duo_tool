...
from calculations import get_inputs, one_time_payment
from utils import check_date_format, check_amount_format
...
def option_one_time_payment(start_date: pd.Timestamp, original_debt: int) -> Tuple[pd.Timestamp, int]:
    with st.expander('Eenmalige extra aflossing', expanded=False):
        ...
        payment_date = st.text_input(
            "Wanneer wil je een extra betaling doen? Vul maand-jaar in: 01-2026",
            value=None
        )
        if payment_date:
        ...
        payment_amount = st.number_input(
            "Wat is het bedrag dat je extra wil aflossen, in hele euro's. Bijv. 5000",
            value=None,
            step=1000
        )
        if payment_amount:
...
def app() -> None:
    ...
    with st.form("basic form"):
        ...
        original_debt = st.number_input(
            'Vul je originele schuldbedrag in hele euros in. Bijvoorbeeld 60000',
            value=10_000,
            step=5000
        )
        if original_debt <= 0:
